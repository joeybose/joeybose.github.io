...
mathematically interesting but has immediate direct impact on real world
...
models are hidden beyond a security layer it also neglects many key aspects of the
threat model that could promote new types of adversarial attacks. In many ways
Adverssarial Attacks can be modelled as an optimization problem but the
resources  available to adversar ---i.e. access to model parametrs,
input/output queries, etc..., are key to understanding the feasibility of any
type of attack.
...
Games"](https://arxiv.org/abs/2007.00720). Motivated by real world use cases
...
Robustnesst](https://adversarial-ml-tutorial.org/). Suppose we are given a
...
function $$\ell$$, used to evaluate $$f$$, an adversarial attack is said to be
*optimal* if,
$$
\begin{equation}
    \textstyle x' \in \argmax_{x'\in \gX}\ell(f(x'),y) \,, \quad \text{s.t.} \quad d(x,x') \leq \epsilon \,.
\end{equation}
$$
...
of adversarial attacks. In the NeurIPS 2020 Adversarial Example Games paper, a
 more challenging setting of **no**n-interactive
...
they cannot adaptively modify their attack strategy based on the output of the
...
* **The target model** $$f_t$$. The adversarial goal is to attack some target
  ...
  hypothesis class $$\mathcal{F}$$. Critically, the adversary has \emph{no access} to
  $f_t$ \emph{at any time}.
...
on the target model by first crafting it on the representative classifier and
then transferring it to the target model. Initially there is no reason to
...
\begin{equation}\label{eq:two_player_game} \tag{AEG}
     \max_{g \in \mathcal{G}_\epsilon}\min_{f_c \in \mathcal{F}}\mathbb{E}_{(x,y) \sim \gD, z\sim p_{z}}[ \ell(f_c(g(x,y,z)),y)] =: \varphi(f_c,g),
\end{equation}
...
$$
\begin{equation}
    (x',y) \sim p_{g} \Leftrightarrow x'= g(x,y,z)\,,\;(x,y) \sim \gD  \,,\,z\sim p_z
    \quad \text{with} \quad
    d(x',x) \leq \epsilon \, .
\end{equation}
$$
...
In practice we can often assume that $$f_t$$ does well on classification on a
non-adversarial dataset. Thus any representative classifier that the adversary
samples should correspondingly be biased towards not only robust classification
on the adversarial samples but also on regular vanilla classification. We can
incorporate this notion and simultaneously reduce the size of the hypothesis
class $$\mathcal{F}$$ by adding a regularizer to overall AEG objective:

$$
\begin{equation}\label{eq:two_player_game_plus_clean}
         \max_{g \in \mathcal{G}_\epsilon}\min_{f_c \in \mathcal{F}}\mathbb{E}_{(x,y) \sim \gD, z\sim p_{z}}[ \ell(f_c(g(x,y,z)),y)] + \lambda \mathbb{E}_{(x,y) \sim \gD_\textrm{ref}}[\ell(f_c(x),y)] =: \varphi_\lambda(f,g).
\end{equation}
$$
...
take this trained generator and feed it inputs leading to adversarial examples
that simultaneously work against *all* models in the hypothesis class. If the
generator is parametrized ---i.e. like a Neural Net, then the inference cost is
extremely cheap at test time and we can generate adversarial attacks on the fly
circumventing cumbersome optimization on new unseen samples.
...
![EX1]({{ "../assets/Poly.pdf" | absolute_url }}){: style="display: block; margin: auto;"}
...
$$
\begin{equation}
    H_{\mathcal{F}}(p_{g}) := \min_{f_c \in \mathcal{F}} \E_{(x,y) \sim p_g} [ \ell(f_c(x),y)]
    \qquad \text{where $\ell$ is the cross entropy loss.}
\end{equation}
$$
Note $$\mathcal{F}$$-entropy is different from the usual definition of Entropy
found in probability textbooks but it's still a useful analogy. One of the main
...
![EX1]({{ "../assets/F_entropy.pdf" | absolute_url }}){: style="display: block; margin: auto;"}
...
![EX1]({{ "../assets/AEG_table2.png" | absolute_url }}){: style="display: block; margin: auto;"}
